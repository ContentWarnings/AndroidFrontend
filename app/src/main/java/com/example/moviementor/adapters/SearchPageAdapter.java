...
public class SearchPageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private static final float GENRE_ROW_BACKGROUND_ALPHA_LIGHT_MODE = 0.9f;
    private static final float GENRE_ROW_BACKGROUND_ALPHA_DARK_MODE = 0.7f;
    ...
    public void onBindViewHolder(final @NonNull RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        if (itemViewType == VIEW_TYPE_HEADER) {
            ...
            Glide.with(genreViewHolder.itemView.getContext()).load(genreData.getGenreBackgroundImageRes()).into(genreViewHolder.genreBackground);

            // Set the opacity of this genre row based on whether or not light or dark mode is
            // enabled
            if (this.isLightMode) {
                genreViewHolder.genreBackground.setAlpha(GENRE_ROW_BACKGROUND_ALPHA_LIGHT_MODE);
            }
            else {
                genreViewHolder.genreBackground.setAlpha(GENRE_ROW_BACKGROUND_ALPHA_DARK_MODE);
            }
